...
_XXX_PKG_DIR = os.path.dirname(__file__)
...
def _version_from_git_describe() -> str:
    ...
    :return: xxx's latest tag
    """
    if not os.path.isdir(
        os.path.join(os.path.dirname(_XXX_PKG_DIR), ".git")
    ):  # noqa: E501
        raise ValueError("not in xxx git repo")
...
def _version():
    # type: () -> str
    """Returns the xxx version from multiple methods

    :return: the xxx version
    """
    version_file = os.path.join(_XXX_PKG_DIR, "VERSION")
    try:
...
_v = re.search(r"[0-9.]+", VERSION)

VERSION_MAIN = _v.group() if _v is not None else VERSION

if __name__ == "__main__":
    print(_version_from_git_describe())
    print(VERSION)
    print(VERSION_MAIN)
